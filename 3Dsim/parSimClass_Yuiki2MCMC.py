...
from ambDist import get_samples, log_prob
from constants import *
import sys

###############################################################################
# CONSTANTS have been moved to constants.py
###############################################################################
    ...
    def set_flow_type(self, FF):
        ...
        **IMPORTANT: this method needs to be updated with each new designed cell geometry, depending
        ...
        try:
            ...
            print("Loading grids ... ")
            grid_dens = si.griddata(np.transpose([zs, rs]), np.log(dens + 1.0), (grid_x, grid_y), 'nearest') 
            # the +1.0 inside the log is to make sure the log-density never goes negative.
...
#        return tempResults
    ...
    def track_molecules(self, endPos, numTraj):
        ...
        global latest_MCMC_vrel, He_MCMC_sample

        #Compute [numTraj] trajectories
        for i in range(numTraj):
            # MCMC search for random helium velocities uses this (variable) velocity,
            #   which represents the difference between v_YbOH and v_flow:
            latest_MCMC_vrel = np.array([np.Inf, np.Inf, np.Inf])
            # Helium MCMC Sample (updated when v_rel ≠≠ latest_MCMC_vrel):
            He_MCMC_sample = np.array([])
...
################################################################################
    ...
    # Generate "num_points" random points in "dimension" that have uniform probability 
    # over the unit ball scaled by "radius" (length of points are in range [0, "radius"]).
    ...
    def get_ambient_velocity(self, temp, v_flow, v_mol):
        ...
        if correc == 0:
            ...
            #                    *np.sin(phi), v0*np.cos(theta))
            # return Vx + xFlow - vx, Vy + yFlow - vy, Vz + zFlow - vz
            v_rel = np.array([vx-xFlow, vy-yFlow, vz-zFlow])
            
            if np.linalg.norm(v_rel - latest_MCMC_vrel) > 15:
                He_MCMC_sample = get_samples(temp, v_rel)
                latest_MCMC_vrel = v_rel
                print(np.linalg.norm(v_rel - latest_MCMC_vrel))
                
            N = len(He_MCMC_sample)
            vMag, theta, phi = He_MCMC_sample[np.random.randint(0, N),:]
...
if __name__ == '__main__':
    ...
    parser.add_argument('-ff', metavar='flows/X_Cell/DS2xyyy.DAT', help='File 
                        containing flowfield from DS2V output') # Specify flowfield
    parser.add_argument('-out', metavar='xyyy.dat', help = 'Output file name, to store 
                        trajectory data') # Specify output filename

    parser.add_argument('--mult', type=float, dest='mult', action='store', 
                        help='Multiplier for the collision cross section') 
    # Specify cross section multiplier (optional)
    
    parser.add_argument('--npar', type=int, dest='npar', action='store', 
                        help='Number of particles to simulate') 
    #Specify number of particles to simulate (optional, defaults to 1)
    
    parser.add_argument('--lite', dest='lite', action='store_true',
                        help = 'Set TRUE if recording trajectories inside the cell is not necessary')

    parser.add_argument('--init_mode', type=int, dest='init_mode', action='store', 
                        help='Code number for initial particle distributions')
    parser.add_argument('--probe_mode', dest='probe_mode', action='store_true', 
                        help='Set TRUE if only particles final locations are needed')
    # parser.add_argument('--correc', dest='correc', action='store', 
                        help='Specify whether to use *corrected* velocity distribution')
    parser.set_defaults(lite=False, mult=5, npar=1, init_mode=0, probe_mode=False, correc=1) 
    #Defaults to LITE_MODE=False, 1 particle and CROSS_MULT=5
