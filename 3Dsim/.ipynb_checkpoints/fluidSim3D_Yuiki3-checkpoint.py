...
#                'f20_lite':10, 'f21_lite':2, 'f22_lite':100, 'f23_lite':200}[file_ext]
    ...
    if file_ext[0] in ['t']:
        ...
        FF = 'flows/T_Cell/DS2{}.DAT'.format(file_ext)
    ...
    #        plt.savefig('images/hist.png')
       ...
       #     plt.savefig("/Users/gabri/Desktop/HutzlerSims/Plots/"+file_ext+"/Dome/phi.png")
        ...
        num = len(f) #number of simulated particles
        
        binx = np.linspace(0,6,13)
        biny = np.linspace(35,88,25)
        print(f[:,0],f[:][0], binx[::-1])
        ext = stats.binned_statistic_2d(f[:,0], f[:,1], f[:,2], 'mean', bins=[binx,biny[::-1]])
        #ext2 = stats.binned_statistic_2d(f[:,0], f[:,1], f[:,2], 'sum', bins=[binx,biny])
        pumptime = stats.binned_statistic_2d(f[:,0], f[:,1], f[:,3], 'mean', bins=[binx,biny])
        #pumptime2 = stats.binned_statistic_2d(f[:,0], f[:,1], f[:,3], 'sum', bins=[binx,biny]) 
        
        #print(ext.statistic*100)
        
        bin2 = np.linspace(0,6,7)
        bin1 = np.linspace(35,88,25)
        
        #figure(num=None, figsize=(60, 60), dpi=80, facecolor='w', edgecolor='k')
        ax1 = sns.heatmap(ext.statistic*100,xticklabels=bin1,yticklabels=bin2,)
        plt.xlabel("z × 2 (mm)")
        plt.ylabel("r × 2 (mm)")
        plt.show()
        ...
        figure(num=None, figsize=(60, 60), dpi=80, facecolor='w', edgecolor='k')
        ax2 = sns.heatmap(pumptime.statistic,xticklabels=biny,yticklabels=binx, square=True)
        plt.xlabel("z (mm)")
        plt.ylabel("r (mm)")
        plt.show()
