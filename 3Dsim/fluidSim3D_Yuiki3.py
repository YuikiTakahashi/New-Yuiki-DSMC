...
crossBB = 3 * 10**(-19) # two-helium cross sectional area in meters-squared from ThO paper- updated by Yuiki in 2020 May
aperture_diameter = 0.0025*2 # aperture diameter in meter, used for converting sccm to reynolds number
...
def set_flow_type(FF):
    ...
    try:
        ...
        if geometry in ['tCell', 'sCell', 'yCell']: # cases added by Ben, 2/19/2020
...
def analyzeTrajData(file_ext, folder, write_file, pos=0.03, write=False, plots=False,rad_mode=False, dome_rad=0.02,debug=False,window=False, radial_size=False, summaryfolder = 'summary',HEATMAP = False):
    '''
...
#    PREV_AP_RAD = 1.167
    ...
    DEFAULT_ENDPOS = {'f':120, 'g':120, 'y':120, 't':180,\
...
#                'f20_lite':10, 'f21_lite':2, 'f22_lite':100, 'f23_lite':200}[file_ext]
    ...
    if file_ext[0] in ['t']:
        ...
        FF = 'flows/U_Cell/DS2{}.DAT'.format(file_ext)
        
        fDens, fVz = set_flow_type(FF)
    ...
    #                debugf.write(' '.join(map(str, [i-1, x, y, z, tim, j] ) )+' 01\n')
            ...
            if (not np.any(f[i])) and f[i-1][2] != DEFAULT_ENDPOS and np.sqrt(f[i-1][0]**2 + f[i-1][1]**2) > 30 and f[i-1][2] < pos:
                #Finds final row of data for adequate particles. Not condensed, but those that
                #went past aperture. Maybe condensed in vacuum chamber?
                x, y, z, vx, vy, vz, tim = f[i-1]
                ...
                j += 1
                number += 1
        print(condensenum, ":# of molecules that did not exit the aperture and stick to the wall of the cell")
        print(number, ":# of molecules that exited the aperture but did not pass the analyze z position and hit the side wall of the whole entire geometry")
        exitnum = 0
        found = False
        for i in range(len(f)):

            # Yuiki understanding; looking for molecules which passed the analyze z position 
        ...
        if debug:
            print("Got to close")
            debugf.close()
        print(exitnum, ":# of molecules that passed the analyze z position")
        print(finals)

        if radial_size == True:

            found = False       
            radialsize = np.zeros((9, exitnum + number,4))
            for k1 in range(9):
                jj = 0
        ...
        if rad_mode == False:
            ...
            numArrived = counts[unique.tolist().index(pos)] + number
        ...
        #If analyzing a *dome* with radius dome_rad centered at aperture
            ...
            numArrived = counts[unique.tolist().index(dome_rad)] + number
    ...
    #    Add the possibility of restricting "final" particles to those in a small window
        ...
        #beam divergence
        spreadB = 180/np.pi * 2 * np.arctan(2.355*np.std(vrs)/(2*np.mean(vzs)))
        print('beam divergence (full angle) (degree): ', spreadB)
    ...
    #        plt.savefig('images/hist.png')
       ...
       #     plt.savefig("/Users/gabri/Desktop/HutzlerSims/Plots/"+file_ext+"/Dome/phi.png")
        ...
        spread2 = 180/np.pi * 2.355*np.std(vrs)/np.mean(vzs)
        gamma = cross * flowrate * sccmSI / (0.05 * vMean)
        #Estimating d_aperture = 0.0025
        reynolds = 8.0*np.sqrt(2.0) * crossBB * flowrate* sccmSI / (np.pi*aperture_diameter * vMean)
        
        print(8.0*np.sqrt(2.0) * crossBB * sccmSI / (np.pi*aperture_diameter * vMean),': conversion factor from SCCM to Reynolds number' )
        #print(crossBB)
        #print(sccmSI)
        #print(vMean)
        ...
        if write == 1 and rad_mode==True:
            with open(directory+summaryfolder+'/'+write_file, 'a') as tc:
                tc.write('  '.join(map(str, [dome_rad0, round(float(flowrate),2), round(gamma,3), round(float(numArrived)/num,3),\
                         ...
                         round(np.std(vzs),3), round(spread2,3), round(np.mean(thetas),3), round(np.std(thetas),3),\
        ...
        #These two are identical except for reporting the dome radius (dome_rad0) versus the plane distance (pos0)
            ...
            with open(directory+summaryfolder+'/'+write_file, 'a') as tc:
                tc.write(str(round(float(pos0),3))+'  '+str( round(float(flowrate),2))+'  '+str( round(gamma,3))+'  '+str( round(float(numArrived)/num,3))+'  '+str(\
                         ...
                         round(np.std(vzs),3))+'  '+str( round(spread2,3))+'  '+str( round(np.mean(thetas),3))+'  '+str( round(np.std(thetas),3))+'  '+str(\
                         round(np.mean(times),3))+'  '+str( round(np.std(times),3))+'  '+str( round(reynolds,2))+'  '+str( round(spreadB,3))+'  '+str( round(1000*median_radius,3))+'  '+str( round(np.mean(vzs),3))+'\n')
        ...
        binx = np.linspace(0,6,7)
        biny = np.linspace(35,88,7)
        #print(useful_pump[:,2])
        ext = stats.binned_statistic_2d(f[:,0], f[:,1], f[:,2], 'mean', bins=[binx,biny])
        #ext2 = stats.binned_statistic_2d(f[:,0], f[:,1], f[:,2], 'sum', bins=[binx,biny])
        pumptime = stats.binned_statistic_2d(useful_pump[:,0], useful_pump[:,1],useful_pump[:,3], 'mean', bins=[binx,biny])
        #pumptime2 = stats.binned_statistic_2d(f[:,0], f[:,1], f[:,3], 'sum', bins=[binx,biny]) 
        
        #print(ext.statistic*100)
        
        bin2 = np.around(binx, decimals=1)
        bin1 = np.around(biny, decimals=1)
        
        #figure(num=None, figsize=(60, 60), dpi=80, facecolor='w', edgecolor='k')
        ax1 = sns.heatmap(ext.statistic*100,xticklabels=bin1,yticklabels=bin2, annot=True)
        plt.xlabel("z (mm)")
        plt.ylabel("r (mm)")
        bottomx, topx = ax1.get_xlim()
        ax1.set_xlim(bottomx-0.5, topx+0.5)
        bottomy, topy = ax1.get_ylim()
        ax1.set_ylim(bottomy+0.5, topy-0.5)
        plt.show()
        #figure1 = ax1.get_figure()    
        #figure1.savefig(directory+summaryfolder+'/'+write_file+'ext.pdf', dpi=400)
        
        #print(pumptime)
        #figure(num=None, figsize=(60, 60), dpi=80, facecolor='w', edgecolor='k')
        ax2 = sns.heatmap(pumptime.statistic,xticklabels=bin1,yticklabels=bin2, annot=True)
        plt.xlabel("z (mm)")
        plt.ylabel("r (mm)")
        bottomx, topx = ax2.get_xlim()
        ax2.set_xlim(bottomx-0.5, topx+0.5)
        bottomy, topy = ax2.get_ylim()
        ax2.set_ylim(bottomy+0.5, topy-0.5)
        plt.show()
        #figure2 = ax2.get_figure()    
        #figure2.savefig(directory+summaryfolder+'/'+write_file+'pumptime.pdf', dpi=400)
...
#    folder = 'InitLarge'
    ...
    if plot == True:
        ...
        if overlap == False:
            ...
            thetas, thetaSig, times, timeSig, reyn, spreadB, medRads, vzmean = f[:,0], f[:,1], \
            ...
            plt.xlabel("Flowrate (SCCM)")
            plt.ylabel("Forward Velocity FWHM")
            plt.show()
            plt.clf()
            
            plt.title("Forward Velocity FWHM vs Reynolds Number")
            plt.errorbar(x=reyn, y=2.355*vzSig, fmt='ro')
            ...
            plt.title("Beam divergence(ThO paper) vs flowrate SCCM")
            plt.errorbar(x=frs, y=spreads, fmt='ro')
            plt.xlabel("flowrate (SCCM)")
            plt.ylabel("Beam divergence(ThO paper) (degree)")
            plt.show()
            plt.clf()
            
            plt.title("FWHM Transverse Velocity vs Reynolds Number")
            plt.errorbar(x=reyn, y=2.355*vRSig, fmt='ro')
            plt.xlabel("Reynolds Number")
            plt.ylabel("FWHM Transverse Velocity (m/s)")
            plt.show()
            plt.clf()
            ...
            plt.title("Mean Forward Velocity vs Flowrate (SCCM)")
            plt.errorbar(x=frs, y=vzmean, yerr=vzSig, fmt='ro')
            plt.xlabel("Flowrate (SCCM)")
            plt.ylabel("Mean Forward Velocity (m/s)")
            plt.show()
            plt.clf()
            
            plt.title("Mean Forward Velocity vs Reynolds Number")
            plt.errorbar(x=reyn, y=vzmean, yerr=vzSig, fmt='ro')
            plt.xlabel("Reynolds Number")
            plt.ylabel("Mean Forward Velocity (m/s)")
            plt.show()
            plt.clf()
            ...
            for overlapfilename in overlap_filelist:
                ...
                plt.figure(4)
                plt.title("Forward Velocity St. Dev. vs Flowrate (SCCM)")
                ...
                plt.figure(10)
                plt.title("Forward Velocity FWHM vs Flowrate (SCCM)")
                plt.errorbar(x=frs, y=2.355*vzSig, fmt=colorlist[colornum])
                plt.xlabel("Flowrate (SCCM)")
                plt.ylabel("Forward Velocity FWHM")
                plt.legend(legendlist)
                ...
                plt.figure(11)
                plt.title("Forward Velocity FWHM vs Reynolds Number")
                plt.errorbar(x=reyn, y=2.355*vzSig, fmt=colorlist[colornum])
                plt.xlabel("Reynolds Number")
                plt.ylabel("Forward Velocity FWHM")
                plt.legend(legendlist)
                ...
                plt.figure(12)
                plt.title("Beam divergence(ThO paper) vs flowrate SCCM")
                plt.errorbar(x=frs, y=spreads, fmt=colorlist[colornum])
                plt.xlabel("flowrate (SCCM)")
                plt.ylabel("Beam divergence(ThO paper) (degree)")
                plt.legend(legendlist)
                ...
                plt.figure(13)
                plt.title("FWHM Transverse Velocity vs Reynolds Number")
                plt.errorbar(x=reyn, y=2.355*vRSig, fmt=colorlist[colornum])
                plt.xlabel("Reynolds Number")
                plt.ylabel("FWHM Transverse Velocity (m/s)")
                plt.legend(legendlist)
                ...
                plt.figure(14)
                plt.title("Mean Forward Velocity vs Flowrate (SCCM)")
                plt.errorbar(x=frs, y=vzmean, yerr=vzSig, fmt=colorlist[colornum])
                plt.xlabel("Flowrate (SCCM)")
                plt.ylabel("Mean Forward Velocity (m/s)")
                plt.legend(legendlist)
                ...
                plt.figure(15)
                plt.title("Mean Forward Velocity vs Reynolds Number")
                plt.errorbar(x=reyn, y=vzmean, yerr=vzSig, fmt=colorlist[colornum])
                plt.xlabel("Reynolds Number")
                plt.ylabel("Mean Forward Velocity (m/s)")
                plt.legend(legendlist)
    ...
    dataSets = {'TimeColumn/plane94_mr.dat' : (0, 'Straight Hole', 'o', '--') ,\
             ...
             'ClusterHCell/window94_mr.dat' : (0, 'Hourglass', 'o', '--'),\
                ...
                'ClusterKCell/window94.dat' : (0, 'de Laval III (K)', 'o', '--'),\
                   ...
                   'InitLarge/window94.dat' : (0, 'Single-stage Cell', 'o', '--'),\

              'InitLargeKCell/window94.dat' : (0, 'de Laval K (i-1)', 'o', '--'),\

                    'InitLarge/plane94.dat' : (1, 'Single-stage Cell', 'o', '--'),\
             ...
             'InitLargeKCell/window111.dat' : (0, 'de Laval K (I1)', 'o', '--'),\
              ...
              'InitAblatFCell/window94.dat' : (0, 'Ablation', 'o', '--'),\
               ...
               'ClusterPCell/plane1026.dat' : (1, 'Slowing Cell P', 'o', '--'),\
              'ClusterPCell/window1026.dat' : (0, 'Slowing Cell P', 'o', '--'),\

                    'TStep100/aperture.dat' : (0, '100step', 'o', '--'),\
                     'TStep100/plane94.dat' : (0, '100step', 'o', '--'),\
                    'TStep100/window94.dat' : (0, '100step', 'o', '--'),\

                'ClusterNCell/plane103.dat' : (1, 'Slowing Cell N', 'o', '--'),\
               'ClusterNCell/window103.dat' : (0, 'Slowing Cell N', 'o', '--'),\
