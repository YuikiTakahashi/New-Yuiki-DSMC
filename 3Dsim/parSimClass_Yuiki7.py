    ...
    def set_flow_type(self, FF):
        ...
        **IMPORTANT: this method needs to be updated with each new designed cell geometry, depending
        ...
        try:
            ...
            elif geometry in ['hCell', 'jCell', 'kCell', 'mCell','wCell']:
            ...
            elif geometry in ['sCell', 'yCell']: # cases added by Ben, 2/19/2020
                grid_x, grid_y = np.mgrid[0.005:0.12:4500j, 0:0.030:1000j]
            elif geometry in ['tCell', 'cCell', 'zCell']: # cases added by Ben, 2/19/2020
                grid_x, grid_y = np.mgrid[0.005:0.18:6750j, 0:0.030:1000j]
            else:
...
###############################################################################
    ...
    def parallel_main(self, outfile):
        ...
        pp = Pool()
        results = pp.map(self.get_trajectory, inputs)

        #results = Parallel(n_jobs=-1,max_nbytes=None,verbose=50, backend='loky')(delayed(self.get_trajectory)(i) for i in inputs)
        ...
        if HEATMAP != True:
            ...
            for i in range(numTraj):
                ...
                while inBounds(x, y, z, cellGeometry, endPos):
                    ...
                    if np.random.uniform(0,1) < self._collProb and no_collide==False: # 1/10 chance of collision
...
################################################################################
    ...
    def random_ball2(self, radius):
        ...
        random_radii = np.random.uniform(0,1)
...
def get_flow_chars(filename):
    ...
    elif filename[13:16] == "DS2" and (filename[16] in ['c'):
    geometry = 'cCell'
    flowrate = 1
    
    elif filename[13:16] == "DS2" and (filename[16] in ['z']):
    geometry = 'zCell'
    flowrate = 1
    
    elif filename[13:16] == "DS2" and (filename[16] in ['w']):
    geometry = 'wCell'
    flowrate = 1
...
def inBounds(x, y, z, cell=None, endPos=0.12):
    ...
    if cell == 'fCell':
        ...
        in0 = r < 0.002 and z > 0.015 and z < 0.035
        in1 = r < 0.00635 and z > 0.035 and z < 0.0881
        in2 = r < 0.0025 and z >= 0.0881 and z < 0.0906
        in3 = r < 0.030 and z >= 0.0906 and z < endPos
        inside = in1 + in2 + in3
        return inside
    
    elif cell == 'cCell':
        in0 = r < 0.002 and z > 0.015 and z < 0.035
        in1 = r < 0.00635 and z > 0.035 and z < 0.0852
        in2 = r < (3.85/2.9)*(0.0852-z)+0.00635 and z >= 0.0852 and z < 0.0881
        in3 = r < 0.0025 and z >= 0.0881 and z < 0.0886
        in4 = r < 0.030 and z >= 0.0886 and z < endPos
        inside = in1 + in2 + in3 + in4
        return inside
        ...
        in1 = r < 0.00635 and z > 0.035 and z < 0.0881
        in2 = r < 0.0025 and z >= 0.0881 and z < 0.0886
        in3 = r < 0.030 and z >= 0.0886 and z < 0.0933
        in4 = r < 0.0085 and z >= 0.0933 and z < 0.1033
        in5 = r < 0.0025 and z >= 0.1033 and z < 0.1038
        in6 = r < 0.030 and z >= 0.1038 and z < endPos
        inside = in1 + in2 + in3 + in4 + in5 + in6
        return inside
...
if __name__ == '__main__':
    for flowname in [ 't010','tv2n','tv3n', 'tv10n','thalfv10n','thalfv25n']:
        simObject = ParticleTracing_Yuiki(flowFieldName='flows/X_Cell/DS2'+ flowname + '.DAT', NPAR=1000,INIT_COND=12,CORRECTION=3)
        outfile='Data/TCellTesting_inicon12_4KHe_newcutoff/'+ flowname + '.dat'
